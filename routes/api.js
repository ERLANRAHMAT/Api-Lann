...
var len = 15
        ...
        var randomTextNumber = random+randomlagi+'--------BETA'+'BETA--BOTZ';
...
router.get('/addapikey', (req, res, next) => {
    ...
    if (apikey != 'Apibeta') return res.json(loghandler.invalidKey)
...
router.get('/cewe/vietnam', async (req, res, next) => {
        var apikeyInput = req.query.apikey
	if(!apikeyInput) return res.json(loghandler.notparam)	
	if (apikeyInput != 'Apibeta')  return res.json(loghandler.invalidKey)
